  ...
  <%@page import="com.kh.board.model.vo.Attachment" %>
    <%@page import="com.kh.board.model.vo.Board" %>
      <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
        <% Board b=(Board)request.getAttribute("b"); Attachment at=(Attachment)request.getAttribute("at"); Used
          ud=(Used)request.getAttribute("ud"); %>
          <!DOCTYPE html>
          <html lang="en">
          ...
          <body>
            ...
            <%@include file="../common/menubar.jsp" %>
              ...
              <section class="boardDetilView_section">
                  ...
                  <div class="used_navi">
                    ...
                    <div class="used_text">
                      ...
                      <div class="priceAndDate">
                        <div class="used_price">가격 : <%=ud.getPrice() %>
                        </div>
                        <div class="used_date">
                          <%=b.getCreateDate() %>
                        </div>
                      ...
                      <div class="used_writer">
                        <%=b.getBoardWriter() %>
                      </div>
                      <div class="used_model">모델 : <%=ud.getModelName() %>
                      </div>

                      <%if(ud.getModelStatus() !=null){ %>
                        <div class="used_trading">상태: <%=ud.getModelStatus() %>
                        </div>
                        <%} %>
                          <%if(ud.getTradingArea() !=null){ %>
                            <div class="used_trading">지역 : <%=ud.getTradingArea() %>
                            </div>
                            <%} %>
                              <%if(ud.getTradingMethod() !=null){ %>
                                <div class="used_trading">거래방법 : <%=ud.getTradingMethod() %>
                                </div>
                                <%} %>
                                  <div class="used_chat">판매자 채팅</div>


                                  <div class="warring">직접결제 시 아래 사항에 유의해주세요.
                                    카페 구매문의 채팅이나 전화 등을 이용해 연락하고 외부 메신저 이용 및 개인 정보 유출에 주의하세요.계좌이체 시 선입금을 유도할 경우 안전한 거래인지
                                    다시 한번 확인하세요.불확실한 판매자(본인 미인증, 해외IP, 사기의심 전화번호)의 물건은 구매하지 말아주세요.
                                    발자취에 등록된 판매 물품과 내용은 개별 판매자가 등록한 것으로서, 발자취는 등록을 위한 시스템만 제공하며 내용에 대하여 일체의 책임을 지지 않습니다.
                                  </div>
                    </div>
                ...
                <div class="backList_btn_area">
                  <%if(u !=null && u.getUserNo()==Integer.parseInt(b.getUserId())){ %>
                    ...
                    <%} %>
                      <a href="<%=contextPath%>/usedBoardList.bo?cpage=1">글목록</a>
                </div>
              ...
              <script>
                ...
                $('.used_chat').on('click', function () {
                  const userNo = $('#userNo').val();
                  const writer = "<%=b.getUserId()%>"
                  if (userNo === "") {
                    alert('로그인 후 이용해주세요')
                    $('.side_login_btn>button').click()
                  } else {
                    searchChatRoomCount(userNo, writer)
                ...
                function searchChatRoomCount(userNo, writer) {
                  $.ajax({
                    ...
                    success: (count) => {
                      ...
                      if (count === 'nnnnn') {
                        ...
                        insertchatRoom(userNo, writer)

                      } else {
                        window.open("<%=contextPath%>/chatpopmsgList.ch?ch=" + count + "&userNo=" + userNo + "&sendUser=<%=b.getBoardWriter() %>", "chatroom", "width=362, height=502")
                ...
                function insertchatRoom(userNo, writer) {
                  $.ajax({
